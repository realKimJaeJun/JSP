	...
	$(document).ready(function(){
		...
		$(document).on('click', '.modify', function(e){
			...
			if(txt == '수정'){
				...
				$.ajax({
					url: '/Farmstory1/proc/commentModifyProc.jsp',
					type: 'POST',
					data: jsonData,
					dataType: 'json',
					success: function(data){
		...
		$('.commentForm > form').submit(function(){
			...
			$.ajax({
				url: '/Farmstory1/proc/commentWriteProc.jsp',
				method: 'POST',
				data: jsonData,
				dataType: 'json',
				success: function(data){
        ...
        <main id="board">
            <section class="view">
                ...
                <div>
                    ...
                    <a href="/Farmstory1/board/proc/deleteProc.jsp?group=<%= group %>&cate=<%= cate %>&no=<%=article.getNo() %>&pg=<%= pg %>" class="btn btnRemove">삭제</a>
                    <a href="/Farmstory1/board/modify.jsp?group=<%= group %>&cate=<%= cate %>&no=<%= article.getNo() %>&pg=<%= pg %>" class="btn btnModify">수정</a>
                    <% } %>
